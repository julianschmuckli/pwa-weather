                ...
                if (json.cod == 200) {
                    ...
                    IndexDB.transaction(["location-data"]).objectStore("location-data").getAll().onsuccess = function (event) {
                        ...
                        if (!is_already_registerd) {
                            ...
                            location_data_location.put(json.id, guid());
...
function deleteCity(id) {
    IndexDB.transaction(["location-data"]).objectStore("location-data").delete(id).onsuccess = function (event) {
        IndexDB.transaction(["weather-data"]).objectStore("weather-data").clear().onsuccess = function (event) {
